...
function logos_setting_admin_page() {
    ...
    <?php
            ...
            if (isset ($_POST['update_slides'])) {
                ...
                $slider_id= esc_attr($_GET['s_id']);
                foreach ($_POST['order'] as $slide_order) {
                    $slide_order = esc_attr($slide_order);
            ...
            if(isset($_POST['add_posts']) && isset($_POST['post_id']) && $_POST['add_posts'] == "Insert" && esc_attr( $_POST['wps_logos_cat'] ) == "unset") {
                global $wpdb, $table_prefix, $post;
                $table_name = $table_prefix.WPS_AWESOME_LOGOS_TABLE;
                $slider_id = esc_attr($_POST['logosid']);
            ...
            } elseif (isset($_POST['add_posts']) && esc_attr( $_POST['add_posts']) == "Insert" && esc_attr($_POST['wps_logos_cat']) != "unset") {
                #Add category slides into logos
                global $wpdb, $table_prefix;
                $table_name = $table_prefix.WPS_AWESOME_LOGOS_TABLE;
                $slider_id = esc_attr($_POST['logosid']);
                $date = date('Y-m-d H:i:s');
                $cat_slug = esc_attr( $_POST['wps_logos_cat']);
                $args = array(
                    'post_type' => 'wps_logos',
                    'post_status' => 'publish',
                    'tax_query' => array(
            ...
            $wps_logos_table = $table_prefix.WPS_AWESOME_LOGOS_TABLE;
                    $where = 'post_id ='.esc_attr($_GET['del_slide']).' AND slider_id ='.$logos_id;
...
<div class="wrap-add-slides wps-awesome-logos">
        ...
        <div id="popup-add-slides" style="display:none"> <!-- div for popup -->
            ...
            if ( $the_query->have_posts() ) {
                ...
                $html .='</div>';
                $html .= '<input type="hidden" name="logosid" value="'.esc_attr($_GET['s_id']).'">';
